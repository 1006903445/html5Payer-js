var xwPlayer = function(obj){
	this.xwPlayerStyle = `
		...
		#xwH5PlayerBox{width: 100%;height: 100%;position: relative;}
		#xwH5PlayerBox .xwH5Player-video-audio{width: 100%;height: 100%;}
		#xwH5PlayerBox video,
		#xwH5PlayerBox audio{width: 100%;height: 100%;}
		...
		#xwH5PlayerBox .xwH5Player-top-box{position: absolute;top: -35px;left: 0;right: 0;width: 100%;height: 40px;}
		#xwH5PlayerBox .xwH5Player-bottom-box{position: absolute;bottom: -40px;left: 0;right: 0;width: 100%;height: 40px;background-color: rgba(0, 0, 0, 0.4)}
		...
		#xwH5PlayerBox .xwH5Player-bottom-box{padding: 5px;color: #fff;}
		#xwH5PlayerBox #xwH5Player-controls{height: 100%;width: 100%;line-height: 30px;display: -webkit-box;display: -webkit-flex;display: -ms-flexbox;display: flex;}
		...
		#xwH5PlayerBox #xwH5Player-controls .xwH5Player-progress-box{-webkit-box-flex: 1;-webkit-flex: 1;-ms-flex: 1;flex: 1;padding: 12px 0px;}
		#xwH5PlayerBox #xwH5Player-controls .xwH5Player-progress{width: 100%;height: 100%;background-color: #313237;border-radius: 10px;}
		#xwH5PlayerBox #xwH5Player-controls .xwH5Player-progress-bar{position: relative;height: 100%;border-radius: 10px;background-color: #1E90FF;overflow:}
		#xwH5PlayerBox #xwH5Player-controls .xwH5Player-progress-bar i{display: block;width: 10px;height: 10px;position: absolute;right: -5px;top: -2px;border-radius: 100%;background-color: #fff;}
		...
		#xwH5PlayerBox #xwH5Player-controls .xwH5Player-audio .xwH5Player-progress-audio-box{display: none;position: absolute;bottom: 20px;left: 5px;width: 20px;height: 70px;background-color: rgba(0, 0, 0, 0.4);padding: 10px 8px;}
		#xwH5PlayerBox #xwH5Player-controls .xwH5Player-audio .xwH5Player-progress-audio{position: relative;width: 100%;height: 100%;background-color: #1E90FF;border-radius: 10px;overflow:}
		#xwH5PlayerBox #xwH5Player-controls .xwH5Player-audio .xwH5Player-progress-audio-bar{position: absolute;top: 0;left: 0;width: 100%;height: 0;border-radius: 10px;background-color: #313237;}
		#xwH5PlayerBox #xwH5Player-controls .xwH5Player-audio .xwH5Player-progress-audio-bar i{display: block;width: 8px;height: 8px;position: absolute;right: -2px;top: 100%;border-radius: 100%;background-color: #fff;}
...
xwPlayer.prototype.videoDom = function(){
	...
	var palyerBox = document.getElementById(DEFAULT.container);
	var loop = DEFAULT.loop ? 'loop' : '';
	var player = '';
	var full = '';
	if(DEFAULT.type == 'video'){
		player = `<video id="xwH5Player" src="${DEFAULT.src}" width="100%" height="100%" ${loop} preload="${DEFAULT.preload}" poster="${DEFAULT.poster}">
			您的浏览器不支持video标签
		</video>`;
		full = `<div class="xwH5Player-full">
		...
		player = `<audio id="xwH5Player" src="${DEFAULT.src}" width="100%" height="100%" ${loop} preload="${DEFAULT.preload}" poster="${DEFAULT.poster}">
			您的浏览器不支持audio标签
		</audio>`;
	}
	var playerHtml = `<div id="xwH5PlayerBox">
		<div class="xwH5Player-video-audio">
			${player}
		</div>
		<div class="xwH5Player-controls-box">
			<div class="xwH5Player-top-box"></div>
			<div class="xwH5Player-left-box"></div>
			<div class="xwH5Player-right-box"></div>
			<div class="xwH5Player-bottom-box">
				<div id="xwH5Player-controls">
					...
					<div class="xwH5Player-audio">
						...
						<div class="xwH5Player-progress-audio-box">
							<div class="xwH5Player-progress-audio">
								<div class="xwH5Player-progress-audio-bar"><i></i></div>
							</div>
						</div>
					</div>
					${full}
	...
	palyerBox.innerHTML = playerHtml;
	self.xwH5PlayerBox = document.getElementById('xwH5PlayerBox');
	self.xwH5Player = document.getElementById('xwH5Player');
	self.xwH5PlayerControls = document.getElementById('xwH5Player-controls');
	self.xwH5PlayerDuration = document.querySelector('.xwH5Player-duration');
	self.xwH5PlayerCurrentTime = document.querySelector('.xwH5Player-currentTime');
	self.xwH5PlayerProgressBar = document.querySelector('.xwH5Player-progress-bar');
	self.xwH5PlayerAudio = document.querySelector('.xwH5Player-audio');
	self.xwH5PlayerProgressAudioBar = document.querySelector('.xwH5Player-progress-audio-bar');
	self.playerCallback();
...
xwPlayer.prototype.volumechange = function(v){
	...
	self.xwH5PlayerProgressAudioBar.style.height = v+'%';
};
// 进度条 时间
xwPlayer.prototype.timeupdate = function(nowTime,allTime){
	...
	if(allTime){
		self.xwH5PlayerControls.className = '';
		self.xwH5PlayerProgressBar.style.width = parseInt(nowTime/allTime*100) + '%';
	}else{
		self.xwH5PlayerControls.className = 'z-hls';
...
xwPlayer.prototype.togglePlay = function(type){
	...
	if(type){
		if(type == 'play'){
			self.xwH5Player.play();
			self.xwH5PlayerBox.className = 'z-play';
		}else if(type == 'pause'){
			self.xwH5Player.pause();
			self.xwH5PlayerBox.className = '';
		}
	}else{
...
xwPlayer.prototype.BindingEvent = function(){
	var self = this;
	document.getElementById('svg-play').onclick = function(){
		self.togglePlay()
	}
	document.querySelector('.xwH5Player-play').onclick = function(){
		self.togglePlay()
	}
	document.querySelector('.xwH5Player-pause').onclick = function(){
		self.togglePlay()
	}
	document.querySelector('.xwH5Player-full').onclick = function(){
		self.fullScreen()
	}
}
...
xwPlayer.prototype.playProgress = function(){
	...
	document.querySelector('.xwH5Player-progress').onclick = function(e){
		e.stopPropagation();
		var progressWidth = document.querySelector('.xwH5Player-progress').clientWidth;
		var currentTime = e.offsetX / progressWidth * xwH5Player.duration;
		xwH5Player.currentTime = currentTime;
	}
	document.querySelector('.xwH5Player-progress-bar i').onclick = function(e){
		e.stopPropagation();
	}
	document.querySelector('.xwH5Player-progress-bar i').onmousemove = function(e){
		e.stopPropagation();
	}
	document.querySelector('.xwH5Player-progress-bar i').onmousedown = function(e){
		e.stopPropagation();
		self.mouseXY.is = true;
	}
	document.querySelector('.xwH5Player-progress-box').onmousemove = function(e){
		e.stopPropagation();
		var progressWidth = document.querySelector('.xwH5Player-progress').clientWidth;
		if(self.mouseXY.is){
			self.mouseXY.x = e.offsetX;
			var progress = self.mouseXY.x / progressWidth * 100;
			self.xwH5PlayerProgressBar.style.width = progress + '%';
		}
	}
	document.querySelector('.xwH5Player-progress-bar i').onmouseup = function(e){
		e.stopPropagation();
		var progressWidth = document.querySelector('.xwH5Player-progress').clientWidth;
		if(self.mouseXY.is && self.mouseXY.x){
			self.mouseXY.is = false;
			var currentTime = self.mouseXY.x / progressWidth * xwH5Player.duration;
			xwH5Player.currentTime = currentTime;
			self.mouseXY.x = null;
			self.mouseXY.y = null;
		}
	}
	document.querySelector('.xwH5Player-progress-box').onmouseleave = function(e){
		e.stopPropagation();
		var progressWidth = document.querySelector('.xwH5Player-progress').clientWidth;
		if(self.mouseXY.is && self.mouseXY.x){
			self.mouseXY.is = false;
...
xwPlayer.prototype.audioProgress = function(){
	var self = this;
	var xwH5Player = self.xwH5Player;
	document.querySelector('.xwH5Player-progress-audio').onclick = function(e){
		e.stopPropagation();
		var progressHeight = document.querySelector('.xwH5Player-progress-audio').clientHeight;
		var volume = parseInt((progressHeight-e.offsetY) / progressHeight*100);
		xwH5Player.volume = self.formatVolume(volume/100);
	}
	document.querySelector('.xwH5Player-progress-audio-bar i').onclick = function(e){
		e.stopPropagation();
	}
}
// 键盘事件
xwPlayer.prototype.selfKeyDown = function(){
	var self = this;
	var xwH5Player = self.xwH5Player;
	document.querySelector('#xwH5PlayerBox').onmousemove = function(e){
		e.stopPropagation();
		self.xwH5PlayerMove = true;
	}
	document.querySelector('#xwH5PlayerBox').onmouseleave = function(e){
		e.stopPropagation();
		self.xwH5PlayerMove = false;
	}
	document.onkeydown = function(e){
		if(self.xwH5PlayerMove){
			var k = e.keyCode;
			if(k == 38){
				var volume = parseInt((xwH5Player.volume + 0.1)*100);
				...
				var currentTime  = xwH5Player.currentTime - 10;
				currentTime = currentTime < 0 ? 0 : currentTime;
				xwH5Player.currentTime = currentTime;
			}else if(k == 39){
				var currentTime  = xwH5Player.currentTime + 10;
				currentTime = currentTime > xwH5Player.duration ? xwH5Player.duration : currentTime;
				xwH5Player.currentTime = currentTime;
			}
		}
	}
